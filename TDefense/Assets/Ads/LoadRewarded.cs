...
public class LoadRewarded : MonoBehaviour, IUnityAdsLoadListener, IUnityAdsShowListener
{
    ...
    {
        if (placementId.Equals(adUnitId) && (int)showCompletionState == (int)UnityAdsCompletionState.COMPLETED)
        {
            print("Rewarded show complete , Distribute the rewards");
            playerUpgrades.adWatched = true;
            playerUpgrades.AddMoneyFromWave();
            playerUpgrades.AdWatched();
        }
    }
